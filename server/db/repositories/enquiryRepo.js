...
export async function getEnquiryById(id) { return (await collection('enquiries')).findOne({ id: normalizeId(id) }); }
export async function updateEnquiryStatus(id, status) { if (!['New', 'Contacted', 'Quoted', 'Follow-up', 'Closed'].includes(status)) return null; await (await collection('enquiries')).updateOne({ id: normalizeId(id) }, { $set: { status } }); return getEnquiryById(id); }
export async function deleteEnquiry(id) { await (await collection('enquiries')).deleteOne({ id: normalizeId(id) }); return { success: true }; }
...
export async function updateContactStatus(id, status) { if (!['New', 'Read', 'Replied'].includes(status)) return null; await (await collection('contacts')).updateOne({ id: normalizeId(id) }, { $set: { status } }); return (await collection('contacts')).findOne({ id: normalizeId(id) }); }
export async function deleteContact(id) { await (await collection('contacts')).deleteOne({ id: normalizeId(id) }); return { success: true }; }
