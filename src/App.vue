<template>
  <div class="min-h-screen flex flex-col bg-white text-ink-900">
    <div v-if="apiLoading" class="fixed top-0 left-0 right-0 z-[60] h-0.5 overflow-hidden bg-brand-100" aria-label="Loading">
      <div class="h-full w-1/3 animate-loading-bar bg-brand-500"></div>
    </div>
    ...
    <template v-if="!isAdminRoute">
      ...
      <a
        ...
        class="fixed bottom-6 right-6 z-40 hidden md:flex items-center gap-2.5 px-4 py-3 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-full shadow-float transition-all hover:scale-105 hover:-translate-y-0.5"
