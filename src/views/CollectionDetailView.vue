<template>
  <div class="collection-detail-page pt-20 pb-16 bg-gradient-to-b from-[#fffdf8] via-[#fffaf2] to-[#f4f1e8]">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div v-else-if="currentCollection" class="space-y-12">
        ...
        <!-- Hero Banner -->
        <div class="relative min-h-80 overflow-hidden rounded-[2rem] border border-[#e8dccb] bg-gradient-to-br from-[#f7ead8] via-[#fffdf8] to-[#e7eee4] text-ink-900 shadow-float">
          <img :src="currentCollection.image" :alt="currentCollection.name" class="absolute inset-y-0 right-0 h-full w-2/3 object-cover opacity-20" />
          <div class="absolute inset-0 bg-gradient-to-r from-[#f7ead8]/95 via-[#fffdf8]/95 to-[#fffdf8]/35"></div>
          <div class="relative z-10 max-w-3xl p-8 sm:p-12">
            <p
            ...
            <h1 class="font-display text-4xl sm:text-5xl font-bold text-ink-950 mb-4">{{ currentCollection.name }}</h1>
            <p class="mb-8 max-w-2xl text-base leading-relaxed text-ink-600">{{ currentCollection.description }}</p>
        ...
        <!-- Subcategories -->
        <div v-if="currentCollection.children && currentCollection.children.length" class="rounded-3xl border border-surface-200 bg-surface-50 p-5 sm:p-6">
          <div class="mb-4 flex items-end justify-between gap-4">
            <div>
              <p class="section-label mb-1">Explore the range</p>
              <h2 class="font-display text-2xl font-bold text-ink-900">Shop by fabric type</h2>
            </div>
            <span class="text-xs font-semibold text-ink-400">{{ currentCollection.children.length }} options</span>
          </div>
          <div class="flex flex-wrap gap-3">
          <router-link
        ...
        <!-- Products Header -->
        <div class="flex flex-col gap-3 border-b border-surface-200 pb-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p class="section-label mb-1">The current edit</p>
            <h2 class="font-display text-3xl font-bold text-ink-900">Available {{ currentCollection.name }} <span class="text-ink-400 font-normal text-xl">({{ pagination.total }})</span></h2>
          </div>
