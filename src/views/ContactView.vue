        ...
        <div class="lg:col-span-5 space-y-6">
          <div class="bg-surface-50 rounded-3xl border border-surface-200 p-8 space-y-6">
            ...
            <div class="pt-4 border-t border-surface-200 flex flex-wrap gap-3">
              <a :href="buildWhatsAppUrl(company?.whatsappUrl || company?.whatsappNumber || '917904154775')" target="_blank" rel="noopener" class="px-5 py-2.5 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-xl transition-all flex items-center gap-2 shadow-sm">
...
const mapLocationQuery = 'Ashok Tex weaving unit, 7/72 Manjanayakkanpatti Pirivu, Esanatham Main Road, Paganatham, Karur, Tamil Nadu 639118';
const googleMapsUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(mapLocationQuery)}`;
const defaultMapUrl = `https://maps.google.com/maps?q=${encodeURIComponent(mapLocationQuery)}&z=17&t=k&output=embed`;
